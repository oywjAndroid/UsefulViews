...
public class RecyclerTabLayout extends RecyclerView {
    ...
    public void setUpWithDiscreteScrollView(DiscreteScrollView discreteScrollView, List<String> titles) {
        DefaultAdapter adapter = new DefaultAdapter(discreteScrollView, titles);
        ...
        // 封装无限循环
        DefaultInfiniteAdapter<DefaultAdapter.ViewHolder> wrapAdapter = DefaultInfiniteAdapter.wrap(adapter);
        setUpWithAdapter(wrapAdapter);
    }

    public void setUpWithAdapter(RecyclerTabLayout.Adapter<?> adapter) {
        mAdapter = adapter;
        mDiscreteScrollView = adapter.getDiscreteScrollView();
        if (mDiscreteScrollView.getAdapter() == null) {
            throw new IllegalArgumentException("DiscreteScrollView does not have a Adapter set");
        }
        mDiscreteScrollView.addScrollStateChangeListener(new DiscreteScrollViewOnPageChangeListener(mDiscreteScrollView,this));
        setAdapter(mAdapter);
        scrollToTab(mDiscreteScrollView.getCurrentItem());
    }

    public void setCurrentItem(int position, boolean smoothScroll) {
        if (mDiscreteScrollView != null) {
            if (smoothScroll) mDiscreteScrollView.smoothScrollToPosition(position);
            else mDiscreteScrollView.scrollToPosition(position);
            scrollToTab(mDiscreteScrollView.getCurrentItem());
            return;
        }
...
//        int top = getHeight() - mIndicatorHeight;
    ...
    /**
     * RecyclerOnScrollListener 监听RecyclerTabLayout的滚动状态。
     */
    ...
    protected static class DiscreteScrollViewOnPageChangeListener implements DiscreteScrollView.ScrollStateChangeListener {
        private final RecyclerTabLayout mRecyclerTabLayout;
        private final InfiniteScrollAdapter mInfiniteAdapter;

        public DiscreteScrollViewOnPageChangeListener(DiscreteScrollView discreteScrollView,RecyclerTabLayout recyclerTabLayout) {
            mRecyclerTabLayout = recyclerTabLayout;
            mInfiniteAdapter = (InfiniteScrollAdapter) discreteScrollView.getAdapter();
        }
        ...
        public void onScrollEnd(@NonNull ViewHolder currentItemHolder, int adapterPosition) {
            LogUtils.d("zcstOywj","adapterPos =="+adapterPosition+" ==> realPos"+mInfiniteAdapter.getRealPosition(adapterPosition));
//            final int realItemCount = mInfiniteAdapter.getRealItemCount();
//            int realPosition = mInfiniteAdapter.getRealPosition(adapterPosition);
//            if (realPosition == 0)

            if (mRecyclerTabLayout.mIndicatorPosition != adapterPosition) {
                mRecyclerTabLayout.scrollToTab(adapterPosition);
            }
        }

        @Override
        public void onScroll(float scrollPosition, @NonNull ViewHolder currentHolder, @NonNull ViewHolder newCurrent) {
            mRecyclerTabLayout.scrollToTab(currentHolder.getAdapterPosition(), scrollPosition, false);
        }
    }
    ...
    public static abstract class Adapter<T extends RecyclerView.ViewHolder>
        ...
        public Adapter(DiscreteScrollView discreteScrollView) {
            mDiscreteScrollView = discreteScrollView;
        }

        public DiscreteScrollView getDiscreteScrollView() {
            return mDiscreteScrollView;
        }
    ...
    public static class DefaultAdapter
        ...
        public DefaultAdapter(DiscreteScrollView discreteScrollView, List<String> titles) {
            super(discreteScrollView);
            mTitles = titles;
        }
...
//            LogUtils.d("zcstOywj", "pos == " + position);
            CharSequence title = mTitles.get(position);
            ...
            public ViewHolder(View itemView) {
                ...
                itemView.setOnClickListener(new OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (pos != NO_POSITION) {
                            InfiniteScrollAdapter adapter = (InfiniteScrollAdapter) getDiscreteScrollView().getAdapter();
                            int realPosition = adapter.getRealPosition(pos);
                            getDiscreteScrollView().smoothScrollToPosition(realPosition);
                        }
                    }
                });
            }
    ...
    /**
     * 主要为了封装DefaultAdapter的无限循环功能。
     *
     * @param <T>
     */
    public static class DefaultInfiniteAdapter<T extends RecyclerView.ViewHolder> extends RecyclerTabLayout.Adapter<T> {

        private static final int RESET_BOUND = 100;
        private Adapter<T> wrapAdapter;
        private LinearLayoutManager layoutManager;
        private int currentRangeStart;

        public static <T extends RecyclerView.ViewHolder> DefaultInfiniteAdapter<T> wrap(
                @NonNull RecyclerTabLayout.Adapter<T> adapter) {
            return new DefaultInfiniteAdapter<>(adapter.getDiscreteScrollView(), adapter);
        }

        public DefaultInfiniteAdapter(DiscreteScrollView discreteScrollView, Adapter<T> adapter) {
            super(discreteScrollView);
            wrapAdapter = adapter;
        }
        ...
        public void onBindViewHolder(T holder, int position) {
            wrapAdapter.onBindViewHolder(holder, getRealPosition(position));
        }

        @Override
        public int getItemViewType(int position) {
            return wrapAdapter.getItemViewType(getRealPosition(position));
        }

        @Override
        public long getItemId(int position) {
            return wrapAdapter.getItemId(getRealPosition(position));
        }

        @Override
        public int getItemCount() {
            return wrapAdapter.getItemCount() == 0 ? 0 : Integer.MAX_VALUE;
        }
        ...
        /**
         * 获取真实ItemCount，即wrapAdapter中的ItemCount。
         *
         * @return itemCount
         */
        ...
        /**
         * 获取当前DefaultInfiniteAdapter的position实际上对应的position。
         *
         * @param position this adapter's position.
         * @return wrapAdapter's position.
         */
        ...
        private int mapPositionToReal(int position) {
            ...
            if (newPosition >= wrapAdapter.getItemCount()) {
                currentRangeStart += wrapAdapter.getItemCount();
                ...
                if (currentRangeStart <= RESET_BOUND) {
                    resetRange(wrapAdapter.getItemCount() - 1);
                }
                return wrapAdapter.getItemCount() - 1;
            } else {
                return newPosition;
            }
        }
